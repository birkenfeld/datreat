      function thrmii(x,pa,thnam,parnam,npar,ini,nopar,params,napar,mbuf)
c     ===================================================================
c
c -------> rousemod_inc <---------
c
c
       ...
       double precision n_monom, del_rho, density, sigtotal, mw_monom
       double precision v_monomer, h_per_mo, phi

       integer iadda
       common/thiadd/iadda

       double precision zpi
       parameter (zpi = 6.283185d0)

       double precision sigma_proton, N_avog
       parameter (N_avog=6.022045d23)          ! [moelcules per mole]
       parameter (sigma_proton=79.904d-24)     ! [cm**2] bound incoherent cross section of proton
...
!:and a width of the transition $p_w=R^2/(2 \,R_w \, R_p)$. Incoherent and coherent contributions are
!:combined according to the scattering and polymer data.
!:Note: N ist the number of segments (beads) for the summation, whereas $N_m$ is the total number of 
...
!: \parnam{n_monom  }{ $N_m$      }{number of monomers in one polymer molecule      }{ -   }         {NO}          
!: \parnam{density  }{ $\rho$     }{effective polymer density                       }{ $g/cm^3   $ } {(y)}  
!: \parnam{del_rho  }{ $\Delta$   }{contrast between polymer and surrounding matrix }{ $ cm^{-2} $ } {(y)}   
!: \parnam{mw_monom }{ $M_w^{mon}$}{molecular weight of monomer                     }{ $ g/mol   $ } {NO}              
!: \parnam{h_per_mo }{ $n(H)/mon$ }{H-atoms per monomer                             }{ -   }         {NO}      
!: \parnam{phi      }{ $\phi$     }{volume fraction of H-ploymer                    }{ -   }         {NO}      
!: 
!: 
!: 
!:\eparameters
...
!        "automatic computation of this ratio " --> don't divide sqt by N ! otherwise the
!                                                   scaling will become wrong
!
         parnam(11)= 'n_monom '    ! number of monomers in one polymer molecule
         parnam(12)= 'density '    ! effective polymer density                        in [g/cm**3]
         parnam(13)= 'del_rho '    ! contrast between polymer and surrounding matrix  in [cm**-2]
         parnam(14)= 'mw_monom'    ! molecular weight of monomer
         parnam(15)= 'h_per_mo'    ! H-atoms per monomer                                            
         parnam(16)= 'phi     '    ! volume fraction of h-polymer                                            
...
! ----------------------------------
       v_monomer = mw_monom/density/N_avog
       sigtotal  = h_per_mo * sigma_proton
             
       rat_ci   =  2*zpi*(n_monom/N*v_monomer*del_rho)**2 /
     *                   (n_monom/N*sigtotal)
     *             *(1.0d0-phi)                                   ! accounts for coh-prefactor    phi*(1-phi) 
                                                                  ! divided by incoherent prefac  phi
! ----------------------------------
...
!         a = fn * a0 * dexp(-qzz*qzz*diff*tau)
     ...
     *                                                        Sqi,Sqti)
       ...
       if(sumnorm.gt.0.0d0) then
          if(sqof0) then
            thrmii = rat_ci*sum -0.333333*sumi
          else
            if(an.gt.0) then
              thrmii = (rat_ci*sum -0.333333*sumi)/
...
!: \euvariables
       ...
       call        parset('rat_ci  ',sngl(rat_ci),iadda,ier) ! ratio coheret/incoherent
...
!: 
!:\algorithm{
!:     V_m   &=& M_w^{mon}/\rho/N_A \cr
!:\sigma_i^{total} &=& n(H)/mon \, \sigma_i^{proton} \cr
!:   \beta   &=& 4\pi ((N_m/N) V_m \Delta)^2 / ((N_m/N) \sigma_i^{total}) \cr
!:       W   &=& Wl4/l^4                        \cr  
!:       xi  &=& 3*kbt/(W*(l^2))                \cr
!:   pt      &=& (R/Rp)^2                       \cr
!:   pw      &=& (R^2)/(2*Rw*Rp)                \cr
...
!:    S_c(q,t) &=& \sum_{n,m,p} \exp( fc (1-\exp(a0)))                   \cr
!:S_{inc}(q,t) &=&  \sum_{n,m,p} \exp( fc (1-\exp(a0)))
!:    th     &=& A_0 \; (\beta S_c(q,t) -1/3 \, S_{inc}(q,t))/(\beta S(q,0) =1/3 \, S_{inc}(q,0))           
!:}
!:
