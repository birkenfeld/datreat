      FUNCTION th_star2 (x, pa, thnam, parnam, npar, ini, nopar ,params,napar,mbuf) 
!     ===================================================               
!                                                                       
! -------> rouse <--------                                              
!                                                                       
!                                    
      ...
      REAL(8) epsilon, diff, wl4, re_arm , extra_frict
      REAL qget, tget, tauget 

      integer :: f_arm, n_arm, n_repeat
      double precision :: pericostar2_sqt, sqt0
...
!        --------------> set the number of parameters                   
         ...
         parnam (5) = 'f_arm'        ! number of arms must be >= 2
...
! ---- calculate theory here -----                                      
      ...
      if(x_is_tau) then                                                                
        sqt0   =       pericostar2_sqt(qz,0d0,f_arm,n_arm,Re_arm,Wl4,extra_frict,n_repeat,diff)                                                       
        th_star2=  a0 * pericostar2_sqt(qz,tau,f_arm,n_arm,Re_arm,Wl4,extra_frict,n_repeat,diff)/sqt0
      else
        th_star2=  a0 * pericostar2_sqt(qz,tau,f_arm,n_arm,Re_arm,Wl4,extra_frict,n_repeat,diff)
      endif 

      if(diff .ne. 0d0) then
        th_star2 = th_star2 * exp(-qz*qz*diff*tau)
      endif 
                                                                       
      RETURN 
      END FUNCTION th_star2



   
double precision function pericostar2_sqt(q,t,f_arm,n_arm,Re_arm,Wl4,extra_frict,n_repeat,diff)
!----------------------------------------------------------------------------------------------
...
!!
 ...
 integer, save                :: f_arm0, n_arm0, n_repeat0
...
eig:  if(  first_run                    &
     ...
     .or.  abs(extra_frict-extra_frict0) > 1e-6  ) then
...
!
        ...
        pericostar2_sqt = (1+f_arm*(s11+(f_arm-1)*s12))/N**2
...
! tentatively add diffusion !
     if(diff == 0d0) then
        ...
        pericostar2_sqt = pericostar2_sqt * exp( -q*q* effective_diffusion * t)
     endif
!


end function pericostar2_sqt
