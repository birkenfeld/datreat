...
!     double precision :: plimit     ! limit of star modes (internal EV value) which are frozen
     ...
     integer          :: modeex     ! mode select exp representation (obsolet)
     integer          :: modeex1    ! mode select exp representation component 1
     integer          :: modeex2    ! mode select exp representation component 2
     integer          :: modeexcc   ! mode select exp representation component cc
...
!          call match_exp (xv,       yv,       np,      me,neq,a     ,r   ,chisq)



          if(ssq > 5d-3) then
            write(6,*)"rpa_test exp model bad match 11, max deviation is:", ssq
          endif
...
!          call nexp_match(t_samples,s_samples,nxpoints,modeex,aexpcc,rexpcc,ssq)
          if(ssq > 5d-3) then
            write(6,*)"rpa_test exp model bad match cc, max deviation is:", ssq
          endif
...
!          call match_exp(t_samples,s_samples,nxpoints,modeex,aexpcc,rexpcc,ssq)

           if(ssq > 5d-3) then
            write(6,*)"rpa_test exp model bad match 22, max deviation is:", ssq
           endif
