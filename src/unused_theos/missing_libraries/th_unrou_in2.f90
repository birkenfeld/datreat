      ...
      FUNCTION th_unrou_inc2 (x, pa, thnam, parnam, npar, ini, nopar ,params,napar,mbuf)
!     ===================================================               
!                                                                       
...
!
      ...
      real th_unrou_inc2, pa, zpi, qq, x
...
! Winkelmittelung
      ...
      if(abs((uuz-uux)*q*q).lt.1d-6) then
        th_unrou_inc2 = a0*exp(-q*q*uuz/2.0d0)
        goto 200
      endif
      ...
      if(uuz.gt.uux) then
        ...
        write(6,*)'sq=',sq
        th_unrou_inc2 = a0*S15AEF(sq,ifail)*sqrt(Pi)*exp(-q*q*uux/2d0)/(2*sq)
!                         Erf
        goto 200
      endif
      ...
      if(uuz.lt.uux) then
        sq = q*sqrt(-uuz/2+uux/2)
        th_unrou_inc2 = a0* S15AFF(sq,ifail)*2*exp(sq**2-q*q*uux/2d0)/(2*sq)
...
200   continue
      ...
      th_unrou_inc2 = frac*th_unrou_inc2 +(1-frac)

      th_unrou_inc2 = th_unrou_inc2 * a01

      call  parset('frac    ',sngl(frac),iadda,ier) 
 
!                                    
      RETURN 
      END FUNCTION th_unrou_inc2
!               
