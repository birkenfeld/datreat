...
! ------------------------------------------------------------------
! 
     sqt0 =  local_reptationB2( q*a, teps,  b, lz)
     sqt  =  local_reptationB2( q*a, t/tau, b, lz)
...
! subroutines and functions entered here are private to this theory and share its variables 
  ...
  function local_reptationB2(q, t, B, L) result(val)  !! exp(x**2)*erf(x) x large series exp
    implicit none
    double precision, intent(in)   :: q, t, B, L
    double precision, parameter    :: A = 0d0
    double precision               :: val
    ...
    double precision               :: x, xa, dec

    xa = (q ** 2 * t + 3d0 * L) * t ** (-1d0 / 2d0) / 6d0
    x  = sqrt(t) * q ** 2 / 6d0
    ...
    if( abs(x) < xlimser ) then
      ...
      val= 72d0 * (sqrt(t) * q ** 4 *  exp((-2d0 * L * q ** 2 * t - 3d0 * L ** 2) &
           / t / 12d0) / 36d0 + sqrt(Pi)  * (q ** 2 * t / 3d0 + L) * q ** 4 * &
           ( dec ) &
           / 72d0 & 
           - sqrt(t) * q ** 4 / 36d0) &
           * B / q ** 4 * Pi ** (-1d0 / 2d0) / L + &
           ...
           exp(-q ** 2 * L / 6d0)  &
         * sqrt(Pi) &
         + (A * L * q ** 2 / 6d0 - A) * sqrt(Pi)) * &
         Pi ** (-1d0 / 2d0) / q ** 4 / L

  end function local_reptationB2
