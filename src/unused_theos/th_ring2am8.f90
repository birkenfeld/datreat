 FUNCTION th_ring2am8(x, pa, thnam, parnam, npar,ini, nopar ,params,napar,mbuf)
!================================================================================
...
! the internal parameter representation 
     ...
     double precision :: r0210       ! reference mean squared displacement at first transition point to medium timed di
     ...
     double precision :: a_cross = 8   ! transition exponent between short and long time diffusion (sharper kink for larg
     double precision :: nuegue      ! (N/N0)**nuegue ==> r021
     double precision :: mseg       ! Segment molwgth
     double precision :: mw         ! Ring molwgth
     double precision, parameter :: mw10 = 10d0         ! Reference Ring molwgth
...
! the reout parameter representation 
     ...
     double precision :: diff_exp, diff0
...
!       --------------> set the parameter names --->
        ...
        parnam ( 2) = 'diff0   '  ! limiting diffusion (NMR value) in [A**2/ns]                                     
        parnam ( 3) = 'r0210   '  ! prefctor to (N/N(10K))^nuegue
        ...
        parnam ( 7) = 'nuegue  '  ! transition exponent between short and long time diffusion (sharper kink for larg
        parnam ( 8) = 'mseg    '  ! mw des Segments                                                 
        ...
        parnam (19) = 'diff_exp'  ! diffusion exponent   
        parnam (20) = 'nue0    '  ! nue0 (Gaussian=0.5)                                              
        parnam (21) = 'ntrans  '  ! structure n transition                                              
        parnam (22) = 'nwidth  '  ! width of that                                             
! >>>>> describe parameters >>>>>>> 
        ...
        th_param_desc( 7,idesc) = "ABS: Guenza ro2 exponent" !//cr//parspace//&
        th_param_desc( 8,idesc) = "ABS: effective segment molwgth (in units of parameter Mw)" !//cr//parspace//&
        ...
        th_param_desc(18,idesc) = "ABS: Ne0 from N/pmin else Ne0" !//cr//parspace//&
        th_param_desc(19,idesc) = "ABS: diffusion exponent D=diff*(Mw10/Mw)**diff_exp" !//cr//parspace//&
        th_param_desc(20,idesc) = "ABS:  std Gaussian exponent for structure " !//cr//parspace//&
        th_param_desc(21,idesc) = "ABS:  transition n form nue0 to nue " !//cr//parspace//&
        th_param_desc(22,idesc) = "ABS:  width of transion form nue0 to nue " !//cr//parspace//&
...
! >>> extract: mw
      ...
      if(ier .ne. 0) then 
         write(*,*)"ERROR: ring2am8 requires presence of parameter mw in data record!"
         return
      endif
      mw        = xh

    if(parameter_is_q .and. parameter_is_t) then
      write(*,*)"th_ring2am8: record parameters of ",iadda," may only contain q or t "
      th_ring2am8 = 0.0
      return
    endif
    if(.not.parameter_is_q .and. .not.parameter_is_t) then
      write(*,*)"th_ring2am8: record parameters of ",iadda," must contain either q or t "
      th_ring2am8 = 0.0
      return
    endif
! 
...
! 
     ...
     n     = nint(mw/mseg)
     r021  = r0210 * (mw/mw10)**nuegue 
     diff  = diff0 * (mw10/mw)**diff_exp
...
! ---- writing computed parameters to the record >>>  
      ...
      call parset('r021    ',sngl(r021)  , iadda, ier)
...
! subroutines and functions entered here are private to this theory and share its variables
       ...
       subroutine Nrouse_ring2am8(q,t,N)
!      ================================================================
!
...
!$OMP END PARALLEL DO
       ...
       end subroutine Nrouse_ring2am8

 end function th_ring2am8
