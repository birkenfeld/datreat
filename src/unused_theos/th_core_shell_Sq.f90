      FUNCTION core_shell3 (x, pa, thnam, parnam, npar,ini, nopar ,params,napar,mbuf)  
!      shell3                                                           
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!                        
      ...
      REAL rhosolv4, phi_nr, x, core_shell3  
      LOGICAL compute_phi 
...
! und Kontraste anbrigen                                                
         ...
         core_shell3  = afactor * (dbc * f1 + dbb * (f2 + f3) ) **2 
                                                                        
!! add the blob-scattering  (xi=xi0*phi**(1/(mu-2)))                    
!       mu=0.6d0                                                        
!       phi=1.0d0                                                       
!                                                                       
!       core_shell3  = core_shell3  +                                                   
...
!     *        blob3(q,xi_blob,phi,Pi0_blob,mu)                         
      ...
      ELSE 
         r = x * xunit 
                                         ! --> Phi Schale 1             
         IF (iphi.eq.1) then 
            core_shell3  = a1 * phi1 (r) 
                                         ! --> Phi Schale 2             
         ELSEIF (iphi.eq.2) then 
            core_shell3  = a2 * phi2 (r) 
                                         ! --> Phi Schale 3             
         ELSEIF (iphi.eq.3) then 
            core_shell3  = a3 * phi3 (r) 
                                         ! --> Phi Schale 1+2+3         
         ELSEIF (iphi.eq.4) then 
            core_shell3  = a1 * phi1 (r) + a2 * phi2 (r) + a3 * phi3 (r) 
         ...
         ELSEIF (iphi.eq.6) then 
            core_shell3  = a1 * phi1 (r) * dbc + (a2 * phi2 (r) + a3 * phi3 (r) &
            ) * dbb                                                     
         ELSE 
            core_shell3  = 0 
         ENDIF 
