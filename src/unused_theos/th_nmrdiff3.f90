 FUNCTION th_nmrdiff3(x, pa, thnam, parnam, npar,ini, nopar ,params,napar,mbuf)
!================================================================================
...
!       --------------> set the parameter names --->
        ...
        parnam ( 4) = 'ths     '  ! thermal occupation factor                                                                  
        parnam ( 5) = 'tauf    '  ! time scale tau-fast                                                                    
! >>>>> describe parameters >>>>>>> 
        th_param_desc( 1,idesc) = "prefactor" !//cr//parspace//&
        th_param_desc( 2,idesc) = "diffusion constant 1 (fast)" !//cr//parspace//&
        th_param_desc( 3,idesc) = "diffusion constant 2 (slow)" !//cr//parspace//&
        th_param_desc( 4,idesc) = "thermal occupation ratio" !//cr//parspace//&
        th_param_desc( 5,idesc) = "timescale by tau-fast" !//cr//parspace//&
...
! mapping to tau1 (fast) and tau2 (slow)
      tau1    =      tauf
      tau2    =      tauf * ( 1d0/thf -1d0 ) 
...
! 
     ...
     p1  = tau1/(tau1+tau2)
     p2  = tau2/(tau1+tau2)
     p2p = 1d0/(a2-a1) *(p1 * qq * diff1 + p2 * qq * diff2 - a1)
     p1p = 1-p2p

     th_nmrdiff3 = ampli * ( p1p*exp(-a1*delta) + p2p*exp(-a2*delta) )

 
 end function th_nmrdiff3
