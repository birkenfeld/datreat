...
C -- SET THE ERROR-HANDLING FOR IMSL VERSION 10 ROUTINES --             
       IERSVR = 0                                                       
       IPACT  = 0                                                       
       ISACT  = 0                                                       
       CALL ERSET(IERSVR,IPACT,ISACT)                                   
C -- FOR ALL SEVERITIES (IERSVR=0) PRINTING IS ON (IPACT=1) AND         
C    AUTOMATIC PROGRAM TERMINATION BY STOP IS OFF (ISACT=0)             
...
!       goto 2222
! Write S(Q,t)
       open(10,file='sqtv3.dgli',status='unknown')
       write(10,*)'L    = ',L
       write(10,*)'p    = ',p
       write(10,*)'zeta = ',zeta
       write(10,*)'temp = ',temp
...
! And S(Q,t) by integration (imsl) funktioniert nicht gut, da imsl aufgibt

      double precision function sqt(qq,t)
!     -----------------------------------
      include 'stiffzero_inc1.f'
      ...
      call DQAND(phi_sst_kernel,2,a,b,errabs,errrel,maxfn,
     *           result,errest)
