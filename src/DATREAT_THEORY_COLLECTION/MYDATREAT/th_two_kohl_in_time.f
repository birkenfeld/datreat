      function th_2kohl_q(x,pa,thnam,parnam,npar,idum,ini)
c     ===================================================
c
c -------> kohl2_q <--------
c
c
C the sum of two KWW functions
 
       implicit none
       
       character*8 thnam,parnam(20)
   
       real*4    x, pa, qq, th_2kohl_q
       ...
       real*8    tau, tau0, beta, a0, a1, epsilon
       real*8    qz
       ...
       real*8 adapint, sum, sum2, result, result2, err, erraccu
       real*8 strexx_kernel, strexx_kernel2
 
       real*8 getyval

       external strexx_kernel
       external strexx_kernel2
...
! communication with Kernel
       ...
       real*8 str_beta2, str_tau02
       common /cstrexx1/Omega, str_beta, str_tau0, str_delta
       common /cstrexx2/str_beta2, str_tau02
...
c        --------------> set the number of parameters
         ...
         parnam(12)= 'f'                 ! f* KWW1 + (1-f) * KWW2
         parnam(13)= 'tau02'             ! KWW time-constant for second KWW function
         parnam(14)= 'beta2'             ! 2nd streched exp
...
! Apply the q-scalings
       if(qz.gt.0.0d0) then
         ...
         str_tau02 = str_tau02 * (1+jlen*qz**qexpt0)/(qz**qexpt0)
         str_beta2 = str_beta2 * (qz**qexpbeta + beta0)
       endif
!
! push these into the parameter section
       call parset('tau0_q  ',sngl(str_tau0),iadda)
       call parset('beta_q  ',sngl(str_beta),iadda)
       call parset('tau02_q  ',sngl(str_tau02),iadda)
       call parset('beta2_q  ',sngl(str_beta2),iadda)
...
!      -------------------------------------
 ...
 100   continue
        ...
        do i=1,ng
         ...
         a = 0
         b = 5.0d0/str_delta
         result  = adapint(strexx_kernel,a,b,epsilon,maxit,erraccu)*2
         result2  = adapint(strexx_kernel2,a,b,epsilon,maxit,erraccu)*2



         sum = sum + gampli(i)*result/(2*Pi)*sqrt(Pi)
         sum2 = sum2 + gampli(i)*result2/(2*Pi)*sqrt(Pi)
...
c              th_2kohl_q = a0*dwf*(a1*sum + 
c     _              (1.d0 - a1)*sum2)
     
       th_2kohl_q = a0*dwf*(a1*sum + 
     _              (1.d0 - a1)*sum2) + abs(bkgr)
       ...
       function strexx_kernel(t)
!      ------------------------
       implicit none

       real*8 strexx_kernel, t
     ...
     *  exp(-1d0/4d0*(str_delta*t)**2) * cos(t*Omega)*str_delta    
       ...
       function strexx_kernel2(t)
!      ------------------------
       implicit none

       real*8 strexx_kernel2, t
