      function th_roufix(x,pa,thnam,parnam,npar,ini,nopar,ps,napar,mbuf)
c     ========================================
c
c -------> nrousefix <--------
c
...
!         a = fn * a0 * dexp(-qzz*qzz*diff*tau)
          dr = diff
          pn = 0.5 + p
          call Nrousef(qzz,tau,temp,dr,xi,N,R, W, l, Sq,Sqt)
          sum = sum + fn*Sqt       
       ...
       if(sumnorm.gt.0.0d0) then
          ...
          th_roufix  = sum/sumnorm
          endif
       else
         th_roufix = 0
       endif 
       th_roufix = th_roufix*a0
...
c
       ...
       subroutine Nrousef(q,t,temp,Dr,xi,N,R, W, l,Sq,Sqt)
!      ==================================================
!
...
!
       ...
       double precision q,t,temp,Dr,xi,R, W,Sq,Sqt, pn
       integer N, nn,mm,p
...
! ----- Anchoring to surface ------------
            ...
            tau_p = 2*W*(1-cos((pi*pn)/dfloat(N)))
            a0    = -t*tau_p
            if(a0.lt.-200.0d0) a0 = -200.0d0
            e0    = 1.0d0-exp(a0)
            
            ffc   = cos((pi*pn*nn)/dfloat(N))* cos((pi*pn*mm)/dfloat(N))
            ffc   = ffc / (pn**2)
