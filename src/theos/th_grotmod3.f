      function th_grotmod3(x,pa,thnam,parnam,npar,idum,ini)
c     ===================================================
!
...
!    
       ...
       real*8 strex_kernel_gm3
       external strex_kernel_gm3
...
! communication with Kernel
     ...
     *       , frac_ch3, tau_ch3, beta_ch3
       integer lmax, nsumr
       common /cstgrot3/Omega, xwidth, str_delta, qc,
     *       diffcm, rotdiff,r1i,r1o,amp1d,tau1d,rconfine,diffconf  
     *       , frac_ch3, tau_ch3, beta_ch3, nsumr
...
c        --------------> set the number of parameters
         ...
         parnam(17) = 'nradshel'          ! number radial shells
c
         th_grotmod3 = 0
         return
       endif
...
!      -------------------------------------
 ...
 100   continue
        ...
        do i=1,ng
         ...
         a = 0
         b = 9.0d0/str_delta
         result  = adapint(strex_kernel_gm3,a,b,epsilon,maxit,erraccu)*2
...
c
       ...
       function strex_kernel_gm3(t)
!      ---------------------------
       implicit none

       real*8 strex_kernel_gm3, t

       double precision :: u1d, u2d, u3d, uuz, uux, sq, ddaws, sumtf
       double precision :: s, dvv, vsum, drr, r11, r22
       integer          :: nsumr, irr
...
!!! hier die Dimesionsfaktoren ueberpruefen!!!!! !!!! 
      ...
      do irr= 1,nsumr          ! summation over radial shells
        r11 = r1i+(irr-1)*drr
        r22 = r1i+irr*drr 
        u2d = rotdiff*(r11/2+r22/2)**2*t
 
        uuz = u1d+u3d
        uux = u2d+u3d
 
        dvv = 4d0*Pi/3d0*(r22**3-r11**3)     
...
!         th_unrou_inc = a0*S15AEF(sq,ifail)*sqrt(Pi)*exp(-q*q*uux/2d0)/(2*sq)
          s = derf(sq)*sqrt(Pi)*exp(-qc*qc*uux)/(2*sq) 
!                         Erf
       else
         sq = qc*sqrt(-uuz+uux)
         if(sq.lt.1d-4) sq=1d-4
!         th_unrou_inc = a0* S15AFF(sq,ifail)*2*exp(sq**2-q*q*uux/2d0)/(2*sq)
         s = ddaws(sq)*2*exp(sq**2-qc*qc*uux)/(2*sq)   
