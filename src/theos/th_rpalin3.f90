 FUNCTION th_rpalin3(x, pa, thnam, parnam, npar,ini, nopar ,params,napar,mbuf)
!================================================================================
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     double precision        :: clfw, clf_alpha
...
! >>>>> describe theory with >>>>>>> 
       ...
       "EXPERIMENTAL subdiff ist heir auf nlin (difflin, keine funktion) BUT NOT YET ESTABLISHED use with caution!"
       th_citation(idesc)     = ""
!       --------------> set the parameter names --->
        ...
        parnam (33+2) = 'qoffs   '  ! qoffset
        parnam (34+2) = 'qscal   '  ! qscale

        parnam (35+2) = 'clfw    '  ! w for clf of lin chain  
        parnam (36+2) = 'clfalpha'  ! alpha for clf of lin chain  
...
! >>>>> describe parameters >>>>>>> 
        ...
        th_param_desc(33+2,idesc) = " qscaling offset " !//cr//parspace//&
        th_param_desc(34+2,idesc) = " qscaling factor " !//cr//parspace//&
        th_param_desc(35+2,idesc) = " rate for clf of lin (0=auto) " !//cr//parspace//&
        th_param_desc(36+2,idesc) = " alpha for clf of lin " !//cr//parspace//&
...
! ---- transfer parameters -----
      ...
      clfw     = abs( pa(35+2))  ! parameter in rpa_laplace
      clf_alpha= abs( pa(36+2))  ! parameter in rpa_laplace
...
i1:  if( mode == 0 ) then     ! normal spin-echo
     ...
     q = (q + qoffs)*qscal
      ...
      allparams(1:55) = [ wl4rous, dble(nrouseff), locr2_b, locr2_a, locr2_ta, locr2_lz, locr2_te, &
                         ...
                         dble(mode), dble(modeex), dble(nxpoints), t0, tmax, rlow, qoffs, qscal, clfw, clf_alpha ]  
...
ilr: if( newcomp_required ) then
      ...
      do i=1,nxpoints
             ...
             if(clfw==0) clfw = Wx
             locrep2 = locrep2 * likhtman_clf(ts,q,re_me,clfw,dble(nlin),lx,clf_alpha)

!             t_samples(i) = ts
!             s_samples(i) = locrep2 * sqt / sqt0 *  exp( -difflin * q*q * (ts)**betadifl ) 
! erstmal hier die subdiffusion (in rouse gelssen da nicht markiert.... Quick and dirty!
             rr = ((exp(-log(r02/diff/6)*nu_subdiff)*r02*ts** nu_subdiff)** a_cross +&
                 (6*diff*ts)**a_cross)**(1d0/a_cross)
             s_samples(i) = locrep2 * sqt / sqt0 *  exp( -(q*q*rr/6d0)**beta_q ) 
