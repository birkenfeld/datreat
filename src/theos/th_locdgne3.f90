 FUNCTION th_locdgne3(x, pa, thnam, parnam, npar,ini, nopar ,params,napar,mbuf)
!================================================================================
...
! >>>>> describe parameters >>>>>>> 
        ...
        th_param_desc( 7,idesc) = "OBSOLETE will automatically be inserted" !//cr//parspace//&
        ...
        th_param_desc(18,idesc) = "prefactor to locrep" !//cr//parspace//&
        th_param_desc(19,idesc) = "tzero shift of locrep" !//cr//parspace//&
...
!   OVERWRITE RE to be consistent with Ne
...
!     Sqdebt = ampdebye * Sqdebt 
 ...
 ! write(*,*)"q,t=",q,t,"  sqt0, sqt=",sqt0,sqt, "debye=", Sqdeb0,Sqdebt
      th_locdgne3 = ampli * sqt/sqt0  * Sqdebt/Sqdeb0
...
!!?????????????????????????????????????????????????????????????????????????
!!?????????????????????????????????????????????????????????????????????????
!    xa= Z/(2*sqrt(w*t))  !!  n oder Z ??? CHECK !
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ae  = exp(1d0)
! write(*,*)"tst: ",q,t,T1,T2
!     val = ( T1**ae + (1d0/Z + b/(3*Z)*T2)**ae ) ** (1d0/ae)
...
!            + (imod* b/(3*Z)*T2)
 ...
 !     val = 1d0/(1+1/Z) * (T1 + (2d0/3d0 + b/3d0 * T2) / Z )
      val = 1d0/(1+1/Z) *  (T1 + (2d0/3d0 + b*(1d0/3d0) * T2) / Z )
