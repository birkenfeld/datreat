      FUNCTION th_polym2 (x, pa, thnam, parnam, npar, ini, nopar ,params,napar,mbuf) 
!     ===================================================               
!                                                                        
...
!       
      ...
      double precision  :: sq, sqt, Ree, l, W, pmin, pwid, wl4
      integer           :: ifix, nsum
...
!        --------------> set the number of parameters                   
         ...
         parnam (14)= 'diff'       ! diffusion (if ==0 the Rouse default is used)
         parnam (15)= 'pmin'       ! mode restriction (minimum mode)                                                      
         parnam (16)= 'pwid'       ! width f mode cutoff                                                                 
         parnam (17)= 'ifix'       ! if =1 chain with fixed end                                                                 
         parnam (18)= 'nsum'       ! overrides default N (rescaling for explicit summation to speed-up)                
...
! ---- calculate theory here -----    
      ...
      Polymer_Wl4       = abs(pa(12))*1d-31   ! Rouse rate converted to m**4/sec 
      ...
      scqt = S_coh_Polymer(Q*1d10,tau*1d-9)       ! one call needed to compute CM_Diffusion
                                                      
      
      if(diff == 0d0) then
        Dcm = 0d0                                 ! use  internal diffusion calculation (rouse default) 
      else   
...
!      diff = diff * 1d-9 / 1d-16
      ...
      if(nsum == 0)  then 
        if(Nx < 100) then
           ...
           nsum = 100
           write(6,*)'nsum set to ', nsum, '(',nx,')'
        endif
      endif 
      Ree  = sqrt((Polymer_l0*1d10)**2*Nx)
      Wl4  =  Polymer_Wl4*1d+31
! Nrousez is in th_rouse4 !
      call NrouseZ(q,tau,temp,diff,wl4,nsum,Ree, W, l,ifix, Sq,Sqt,pmin, pwid)
      scqt = scq0*Sqt/Sq
