      FUNCTION th_star2a (x, pa, thnam, parnam, npar, ini, nopar ,params,napar,mbuf) 
!     ===================================================               
!                                                                       
! -------> rouse <--------                                              
!                                                                       
...
!        --------------> set the number of parameters                   
         ...
         parnam (5) = 'onearm'       ! contrast only on one of the twoi arms
         parnam (6) = 'fixcent'      ! fix the center
         parnam (7) = 'fric0'        ! friction modulation fmod= fric0+fricn*i**nfric
         parnam (8) = 'fricn'        !      " 
         parnam (9) = 'nfric'        !      " 
...
! ---- calculate theory here -----                                      
      ...
      if(x_is_tau) then                                                                
        sqt0   =       pericostar2a_sqt(qz,0d0,f_arm,n_arm,Re_arm,Wl4, fric0, fricn, nfric)                                                       
        th_star2a=  a0 * pericostar2a_sqt(qz,tau,f_arm,n_arm,Re_arm,Wl4,fric0,fricn, nfric)/sqt0
      else
        th_star2a=  a0 * pericostar2a_sqt(qz,tau,f_arm,n_arm,Re_arm,Wl4, fric0, fricn, nfric)
      endif 

      if(diff .ne. 0d0) then
        th_star2a = th_star2a * exp(-qz*qz*diff*tau)
      endif 
...
double precision function pericostar2a_sqt(q,t,f_arm,n_arm,Re_arm,Wl4, fric0, fricn, nfric)
!----------------------------------------------------------------------------------------------
...
if(onearm == 0) then
        s12 = 0         ! the "cross term"   !! OMIT IF ONLY ONE ARM IS LABELLED
         if(f_arm > 1) then
!$OMP PARALLEL DO REDUCTION(+:S12) 
          do i=2,n_arm+1
!           if(fixcenter==1 .and. abs(Eigenvectors(1,i))>1d-3/N) cycle
...
!$OMP END PARALLEL DO
        ...
        pericostar2a_sqt = (1+f_arm*(s11+(f_arm-1)*s12))/N**2
...
!        pericostar2a_sqt = pericostar2a_sqt * exp( -q*q* effective_diffusion * t)
!     endif
!


end function pericostar2a_sqt

END FUNCTION th_star2a
