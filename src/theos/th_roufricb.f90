      FUNCTION th_roufricb (x, pa, thnam, parnam, npar, ini, nopar ,params,napar,mbuf) 
!     ===================================================               
!                                                                       
! -------> rouse <--------                                              
!                                                                       
...
!        --------------> set the number of parameters                   
         ...
         parnam (13) = 'bgr_clas'     ! discriminator according to bgr_class
         parnam (14) = 'bgr_frac'     ! fraction of bgr in this class
...
!
 ...
 !                                                                         
      ...
      if(x_is_tau) then                                                                
        sqt0   =       pericoroufricb_sqt(qz,0d0,f_arm,n_arm,Re_arm,Wl4, fric0, fricn, nfric)                                                       
        th_roufricb=  a0 * pericoroufricb_sqt(qz,tau,f_arm,n_arm,Re_arm,Wl4,fric0,fricn, nfric)/sqt0
      else
        th_roufricb=  a0 * pericoroufricb_sqt(qz,tau,f_arm,n_arm,Re_arm,Wl4, fric0, fricn, nfric)
      endif 

      if(diff .ne. 0d0) then
        th_roufricb = th_roufricb * exp(-qz*qz*diff*tau)
      endif 
      ...
      th_roufricb = th_roufricb * (1d0-bgr_frac) + bgr_frac
!     -----------------------------------------------------
 

CONTAINS
   
double precision function pericoroufricb_sqt(q,t,f_arm,n_arm,Re_arm,Wl4, fric0, fricn, nfric)
!----------------------------------------------------------------------------------------------
...
!$OMP END PARALLEL DO
        ...
        pericoroufricb_sqt = (1+f_arm*(s11+(f_arm-1)*s12))/N**2
...
!        pericoroufricb_sqt = pericoroufricb_sqt * exp( -q*q* effective_diffusion * t)
!     endif
!


end function pericoroufricb_sqt
