...
!
!  change: NonGaussian series 1-Q**2 replaced by exp(-Q**2)
!==================================================================================
!
...
!==================================================================================
  ...
  double precision :: nonGaussianity_floor_B = 0d0

  integer          :: maximum_rouse_n = 300
...
CONTAINS
  ...
  function reptation2_sqt(q,t, N, lseg, Ne, Re, wl4, alpha0, talphamax,talphawd) result(sqr0t)
    ...
    W         = Wl4 / lseg**4
    Nrsum     = min(maximum_rouse_n,nint(Ne))
...
!    val = 1d0/(1+1/Z) *  (T1 + (2d0/3d0 + (1d0/3d0) * T2) / Z )
  ...
  function reptationlik_sqt(q,t, N, lseg, Ne, Re, wl4, alpha0, talphamax,talphawd) result(sqr0t)
    implicit none
    double precision, intent(in)  :: q     ! the Q-value 
    double precision, intent(in)  :: t     ! the time
    double precision, intent(in)  :: N     ! total number of segments in chain
    ...
    sq_locrep(1)   =  local_reptationlik( q, teps          ,  lseg, W, n, ne)
    sq_locrep(2)   =  local_reptationlik( q, max(t,teps)   ,  lseg, W, n, ne)

    sqr0t = sq_rouse * sq_locrep

  end function reptationlik_sqt
  ...
  function local_reptationlik(q, t, lseg, W, n, ne) result(val)   
    implicit none
    double precision, intent(in)   :: q, t
    double precision, intent(in)   :: lseg     ! lsegment
    double precision, intent(in)   :: W        ! Rouse rate
    ...
    double precision, parameter    :: Pi = 4*atan(1d0)
    double precision, parameter    :: xlimser = 15d0  ! where to switch erf expression to series expansion
    double precision, parameter    :: alpha_lik = 1.5d0
...
!    T2  = T2 * likhtman_clf(t,q,ne,W,n,lseg,alpha_lik)

!    val = 1d0/(1+1/Z) *  (T1 + (2d0/3d0 + (1d0/3d0) * T2) / Z )
    val = 1d0/(1+1/Z*b_z_floor) *  (T1 + (1d0 + b_mcdefault*b_fluctuation * T2) / Z * b_z_floor )

    val =  val * likhtman_clf(t,q,ne,W,n,lseg,alpha_lik)


  end function local_reptationlik
...
!       fqq  = 1d0 -q**2 * rmm/12d0 * alpha(t)       !! see Guenza PHYSICAL REVIEW E 89, 052603 (2014) Eq(4)
       fqq  = (exp ( -q**2 * rmm/12d0 * alpha(t)  ) + nonGaussianity_floor_B) / &
              (      1d0                            + nonGaussianity_floor_B)  !! modified exp: see Guenza PHYSICAL REVIEW E 89, 052603 (2014) Eq(4)
...
function likhtman_clf(t,q,ne,W,n,l,alpha) result(sumlik)
  implicit none 
  double precision, intent(in ) :: t     ! time
  double precision, intent(in ) :: q     ! momentum transfer
  double precision, intent(in ) :: ne    ! entaglement ne
  double precision, intent(in ) :: W     ! Rouse rate
