 FUNCTION th_locredp3(x, pa, thnam, parnam, npar,ini, nopar ,params,napar,mbuf)
!================================================================================
...
!       --------------> set the parameter names --->
        ...
        parnam (15) = 'wlocr   '  ! width of cutoff                                                                   
! >>>>> describe parameters >>>>>>> 
        ...
        th_param_desc(15,idesc) = "width local reptation cutoff" !//cr//parspace//&
...
! ------------------------------------------------------------------
! 
     sqt0 =  local_reptationdr2( q, t0_locrep, a, W, n, ne,  b)
     sqt  =  local_reptationdr2( q, t,         a, W, n, ne,  b)

     f    =  fermi(t,t0_locrep,w_locrep)  
     sqt  =  f * sqt0 + (1d0-f) * sqt
...
!     Sqdebt = ampdebye * Sqdebt 
 ...
 ! write(*,*)"q,t=",q,t,"  sqt0, sqt=",sqt0,sqt, "debye=", Sqdeb0,Sqdebt
     ...
     th_locredp3 = ampli * ( sqt + Sqdebt)/(sqt0 +Sqdeb0)
...
!    val = (T1+T2)/T3
    ...
    ec1 = erfc(n/(2*sqrt(w*t))+a**2*q**2*sqrt(w*t)/6)
    ec2 = erfc(a**2*q**2*sqrt(w*t)/6) 
    if( isnan(ec1) ) ec1 = 1d0 - erf(n/(2*sqrt(w*t))+a**2*q**2*sqrt(w*t)/6)
    if( isnan(ec2) ) ec2 = 1d0 - erf(a**2*q**2*sqrt(w*t)/6) 
  ...
  function local_reptationdr2(q, t, a, W, n, ne, b) result(val)   !! with series expansion
    implicit none
    double precision, intent(in)   :: q, t
    double precision, intent(in)   :: a        ! step (entanglement?) length
    double precision, intent(in)   :: W        ! (Rouse?) rate
