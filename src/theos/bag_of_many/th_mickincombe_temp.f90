...
!         th_mickincomb = sum * intensity * rho0 * 1d4 
         v0            =  Mw_core/(den_coremat) + Mw_shell/(den_shellmat)
         th_mickincomb = sum * intensity * 1d4 / v0
...
!      ---------------------                                            
       ...
       double precision :: y1, y2, y1e, y2e

       xh = x

       y1 = (-(x -tau_off)/tau_t1)
       if(y1.gt.0d0)    y1 = 0d0
       if(y1.lt.-50d0)  y1 = -50d0
       
       y2 = (-(x-tau_off)/tau_t2)
       if(y2.gt.0d0)    y2 = 0d0
       if(y2.lt.-50d0)  y2 = -50d0
       


       temp_t = amp_T*(1 - exp(y2))**4.2*(exp(y1)) + 298.0d0

!       write(6,*)'t: ',x,y1,y2,temp_t
...
!         endif
! New 14.06.12
        if (amp_T.ne.0.0) then 
       gamma = gamma0*298.0/temp_t 
       else 
       gamma = gamma0
       end if
 
!       write(6,*)'x, gamma .........',x, gamma      
!      write(6,*)'amp_T.............',amp_T      
!      write(6,*)'tau_t1............',tau_t1     
!      write(6,*)'tau_t2............',tau_t2      
!      write(6,*)'tau_off...........',tau_off      
!      write(6,*)'beta   .......... ',beta
!      write(6,*)'coscal .......... ',coscal
!      write(6,*)'gamma  .......... ',gamma
...
!     write(6,*)'GpN, gamma, p, beta, rho(1) = ',GpN, gamma, p, beta, rho(1)
...
!    Dormidontova MM 32 7630 (1999)                                     
       ...
       double precision function previous_ka_pN(k) 
!      -------------------------------                                  
       implicit none 
...
!       ka_pN = 1.d0/tau0*exp(-beta*k**(0.5d0)) 
  ...
  previous_ka_pN = 1.d0/tau0*4*3.14159265*k**(1.0d0/3.0d0)  
...
!       
!       if (amp_T.ne.0.0) then 
!       ka_pN = ka_pN/298.0*temp_t !! Insertion constant scales with viscosity - proportional to 1/T
!       else 
!       ka_pN = ka_pN
!       end if
...
!      -------------------------------                                  
       ...
       double precision gamma0
       double precision :: y1, y2, y1e, y2e
       double precision :: a1, a2, a3, ax, pdep   
...
! Temperature dependence
       ...
       y1 = (-(xh -tau_off)/tau_t1)
       if(y1.gt.0d0)    y1 = 0d0
       if(y1.lt.-50d0)  y1 = -50d0
       
       y2 = (-(xh-tau_off)/tau_t2)
       if(y2.gt.0d0)    y2 = 0d0
       if(y2.lt.-50d0)  y2 = -50d0
   ...
   temp_t = amp_T*(1 - exp(y2))**4.2*(exp(y1)) + 298.0d0
    ...
    ax =  temp_t - 273.0d0 !! viscosity expressed in C rather than K
    eta = 0.01066*ax**(7.166*ax**(-0.454))*1.0d-3   !!0.01066*Col(T)^(7.166*Col(T)^(-0.454) )// in Pas
    pdep = (1.0 + k**(1.0d0/3.0d0))**2/(k**(1.0d0/3.0d0))
    
       
       if (amp_T.ne.0.0) then                            !! SIAM J. APPL. MAT Vol. 72, No. 1, pp. 201
      ka_pN = 1.0d0/tau0*2.0*8.314*temp_t/(3.0*eta)*pdep !! Insertion constant scales with 1/viscosity, proportional to T 
       else
      ka_pN = 1.d0/tau0*4*3.14159265*k**(1.0d0/3.0d0)  
       end if
