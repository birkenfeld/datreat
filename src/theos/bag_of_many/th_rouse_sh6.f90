      FUNCTION th_nrouse_sh6 (x, pa, thnam, parnam, npar, ini, nopar ,params,napar,mbuf) 
!     ================================================================================               
!                                                                       
! -------> nrouse_sh6 <--------
! selfhealing multimodal distribution mix of Rouse
!    from parent neouse_sh2
!    modification: add a switch factor for the internal dynamics
!    to be able to compute the diffusion effect only        
!                                                                       
!                
      implicit none  
      ...
      double precision :: temp, tau, q, wl4, l , taulim, pfrac, ifrac
...
!         --------------> set the number of parameters
         ...
         parnam(10)= 'pfrac   '   ! fraction of mode extension, default 0 or 1
         parnam(11)= 'ifrac  '   ! internal dynamics factor: normal=1, off=0
...
!  ---- calculate theory here -----
       ...
       call Nrouse_sh6(qz,tau,diffscal,Wl4,N,R, m_aver, wcut,fpos, l,dr,taulim, pfrac,ifrac, Sq,Sqt)
 
       if(sqof0) then
          th_nrouse_sh6 =  a0 * Sqt
       else
          th_nrouse_sh6 =  a0 * Sqt/Sq
       endif
       ...
       subroutine Nrouse_sh6(q,t,DrScal,Wl4,Nb,R,m_aver,wcut,fpos,l,Daver,taulim,pfrac,ifrac,SqAve,SqtAve)
!      ===========================================================================
!
...
!    pfrac  ---> mode cut off extension region
!    ifrac  ---> internal dynamics switch factor (1=normal, 0=off)
! Output parameters:
...
!
       ...
       double precision, intent(in)     ::  pfrac
       double precision, intent(in)     ::  ifrac
       ...
       double precision :: survival_time
...
! ---- Do the sums -----
           do nn = n1,n2
            do mm = n1,n2
              ...
              arg20= 0
              do p = 1,N
                 rate_p = 2*W*(1-cos((pi*p)/dfloat(N)))  * ifrac
                 if(taulim < 0d0 ) then
                    rate_p = rate_p + 1/abs(taulim) ! try the Freed Scheme funktioniert nicht
                 endif
!####new               if(1d0/rate_p > taulim) cycle
!!              wmode = exp(-1d0/(rate_p*taulim))
!####newnewnew
                if(taulim > 0d0) then
                   if(mblocks > 1) then
                      survival_time = taulim / ( (mblocks-1d0) * (1d0-0.5d0*pfrac/p) )
                      wmode = exp(-1d0/(rate_p*survival_time))        !####new
                   else
                      wmode = 1d0
                   endif   
