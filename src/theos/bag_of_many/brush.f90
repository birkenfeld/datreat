!*ds                                                                    
!*ed                                                                    
      SUBROUTINE thbrush (q, tau, r0, sigmar0, rho_innen, ratio, k_max, &
      ll_max, iflcorr, ninter, aiqt, aiqt0, sq, ier)                    
!                                                                       
! **** input parameter:                                                 
...
! * save-commons                                                   *    
! ******************************************************************    
      COMMON / isave / kmold1, ninterold, iflcold, kmold3, llmold3 
...
!      ===============================================                  
         ...
         elam, delam, hmax, maxit, maxfun, monit, report, ifail)        
...
! ******************************************************************    
      ...
      IF (q.ne.qold2.or.r0.ne.r0old2.or.sigmar0.ne.sigmaold.or.rho_innen&
     &.ne.rhoold) then                                                  
...
! --------> so jetzt kommen die exp(-tau/tau_i) koeffizientenarrays --- 
      ...
      IF (k_max.ne.kmold3.or.ll_max.ne.llmold3.or.newsol.or.newfl.or.q.n&
     &e.qold3) then                                                     
...
! --- look for bad m value ---                                          
      IF (m.lt.0) then 
      WRITE (6,  * ) ' error: bsjn called with neg m =', m, ' bsjn = 0' 
         bsjn = 0 
         RETURN 
      ENDIF 
!                                                                       
...
!        -----> use limiting formula                                    
         bsjn = (z**m / f) * (1d0 - z**2 / (2 * (2 * m + 3) ) + z**4 /  &
         (8 * (2 * m + 3) * (2 * m + 5) ) )                             
...
!                                                                       
    ...
    1 END DO 
      ...
      bsjn = fn * dsin (z) - ( - 1) **m * fmnm1 * dcos (z) 
!                                                                       
      RETURN 
      END FUNCTION bsjn                             
