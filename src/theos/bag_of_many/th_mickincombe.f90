       function th_mickincombe(x, pa, thnam, parnam, npar,ini, nopar ,params,napar,mbuf)
!     =====================================================      
!
!      micellkinetics
...
!        --------------> set the number of parameters                   
         ...
         parnam(5) = 'beta'              ! [--> beta=factor in front of p**(3/2) term of micelle energy]
         ipa0  (5) = .TRUE. 
         ...
         parnam(9) = 'cte_blob'          ! prefactor for blob scattering
         ipa0  (9) = .FALSE. 

         parnam(10) = 'alpha'            ! prefactor for term in GpN default=1
         ipa0  (10) = .TRUE. 

         parnam(11) = 'epsil_mc'         ! scaling of Mw_core
         ipa0  (11) = .FALSE. 
                                                                        
                                         ! rho0   ACHTUNG aus Datenparameterfeld                      
         parnam(12) = 'epsil_ms'         ! scaling of Mw_shell
         ipa0  (12) = .FALSE.
                                  
         parnam(13) = 'nue'              ! exponent for rho**(-nu) 
         ipa0  (13) = .FALSE.
...
!old       call D02EJF(XS, XEND, kmax, Y, FCNC, D02EJY, TOL, RELABS, OUTPUCT, 02EJW, W, IW, IFAIL)                                
       call DDRIV1(kmax, XS, Y, FCNC, XEND,-1 , TOL, W, IW,IFAIL)                                                                
        write(6,*)'ifail = ',ifail 
        close(10) 
        close(11) 
        close(12) 
...
!         write(6,*)'Mws=',Mw_core,Mw_shell
         ...
         den = 0      
         sx  = 0                         
         do j=2,kmax                           ! unimere ausgelassen !?
          den = den + y(j)
          sx  = sx + j*y(j)
         enddo
...
!         th_mickincombe = sum * intensity * rho0 * 1d4
         v0            =  Mw_core/(den_coremat) + Mw_shell/(den_shellmat)
         th_mickincombe = sum * intensity * 1d4 / v0
       ...
       th_mickincombe = 0
       write(6,*)'th_mickincombe: should not be here'
...
!>!
          ...
          GpN = beta*p**(3.0d0/2.0d0)+gamma*p**(2.0d0/3.0d0)   &
      &       - p*(beta+gamma)                                 &
...
!!
!!
!!       tau = tau0*k**(-alpha)*phi(1)**(-2.0d0/3.0d0)
!!         and  
!!       ka_pN = 1.d0/tau*exp(-k**(0.5d0))
