 FUNCTION th_rpalir2(x, pa, thnam, parnam, npar,ini, nopar ,params,napar,mbuf)
!============================================================================
...
! the recin parameter representation 
     ...
     integer          :: nlin_cc    ! number of segments of ring polymer matrix 
...
! >>>>> describe theory with >>>>>>> 
       ...
       th_explanation(idesc)  = " applies danamic rpa to the scattering function for a "//cr//parspace//&
                                ...
                                " tentative change internal assignment ring=1, lin=2" !//cr//parspace//&
...
!             plin0cc   = nring_cc  * Debye_qnl(q, nring_cc, l) 
             plin0cc   = nlin  * Debye_qnl(q, nlin, llin) 
             plincc    = plin0cc * fqt
             s_samples(i) = fqt   


!! write(*,'(a,5(a,f12.6))')"T2 rin sq: ","sqtres(1)=",sqtres(1)," nring_cc=",dble(nring_cc),"  plin0=",&
!!             Debye_qnl(q, nring_cc, lring) * nring_cc,"   t=", ts, "  fqt=", fqt
...
!         write(*,*)"WARNING  WARNING  WARNING  WARNING  WARNING  WARNING  WARNING  WARNING  WARNING  WARNING"
  ...
  !! of module rpa_laplace
   ...
   ! componente 2    = ring-polymer
        phi1              =   phiring   ! volume fraction of polymer component 1
        phi2              =   philin    ! volume fraction of polymer component 2
        
        Scc00             =   plin0cc   ! unperturbed structure factor S(Q) of "matrix" polymers
        S0011             =   pring0    ! unperturbed structure factor S(Q) of polymer 1
        S0022             =   plin0     ! unperturbed structure factor S(Q) of polymer 2
...
!        write(6,*)"numeric integration of log spaced oscillating integral.."
        ...
        call sel_scomp(4)                     !!! CHECK ! 3 oder 4 ??
     ...
     sqt  = ss11  * bring*bring+ 2*blin*bring * ss12  +  ss22  * blin*blin 
     sqt0 = ss110 * bring*bring+ 2*blin*bring * ss120 +  ss220 * blin*blin 
...
1111     continue
             call  ring2am7(q,t, nring, ring_ne0, lring, ring_nue, ring_nue0, ring_wl4,  &
                      ...
                      diff,alpha,beta,r021,r022,a_cross, &
                      sqtres)                       !??
            th_rpalir2 = sqtres(2)/sqtres(1)

            write(*,*)"T1:",q,t,sqtres, th_rpalir2 
