...
implicit none
 ...
 character(len=commlen)    :: paramater_description(mtpar)
 character(len=commlen)    :: theory_citation
 character(len=namelen)    :: recparin(nrecpar)
 character(len=commlen)    :: recparin_description(mrecpar)
 character(len=namelen)    :: recparout(nrecpar)
...
d3: do
       ...
       parameter_description(npar) = adjustl(trim(buf(9:)))  
...
d4: do
       ...
       recparin_description(nrecpar) = adjustl(trim(buf(9:)))  
...
CONTAINS
  ...
  subroutine make_the_header
   ...
   write(out,'(a)') "  real    :: pa (*) "
   write(out,'(a)') "  real    :: x "
   write(out,'(a)') "  integer :: mbuf, nparx, ier, ini, npar"
  ...
  subroutine make_init_section
    ...
    integer :: i
      ...
      write(out,'(a)')     "            WRITE (6,  (' theory: ',a8,' no of parametrs=',i8,' exceeds current max. = ',i8)) thnam, nparx, npar"
...
l2:   do i=1,npar
        write(out,'(a,i2,3a)')     '        th_param_desc(',i,',idesc) = "',trim(paramater_description(i)),'" !//cr//parspace//&'
      enddo l2
...
HIER
      write(out,'(a)')     "      q   = pa(4)
      write(out,'(a)')     "      CALL getpar ('q       ', xh,nopar ,params,napar,mbuf, ier)
      write(out,'(a)')     "      if(ier.eq.0) q = xh
      write(out,'(a)')     "
      write(out,'(a)')     "      tau = pa(5)
      write(out,'(a)')     "      CALL getpar ('tau     ', xh,nopar ,params,napar,mbuf, ier)
      write(out,'(a)')     "      if(ier.eq.0) tau = xh
      write(out,'(a)')     "
      write(out,'(a)')     "
      write(out,'(a)')     "      diffusion   = abs(pa(2))
      write(out,'(a)')     "      lz          = abs(pa(3))
      write(out,'(a)')     "      mode        = nint(pa(6))
      write(out,'(a)')     "
      write(out,'(a)')     "      theta0      = abs(pa(7))
      write(out,'(a)')     "      wtextur     = abs(pa(8))
      write(out,'(a)')     "
