...
! ----- search for the scans to be selected -----
      ...
      dimension irecv(size(inpar)),irecn1(size(inpar))
!
      l = 0
      do 1001 ic = 1,nkurv
        num = irecv(ic)
...
!
      ...
      dimension ifrec(size(inpar)),irecn1(size(inpar))
!
      l = 0
      do 1001 ic = 1,nfkurv
        num = -ifrec(ic)
...
!       this will make it easier for future releases to incorporate C without all these commonblocks RB
       thval =  theory_x(x,ntheos,nthtab,multflg,thparx,thrapar,thramin,thramax, & 
                nopar(iadda),params(:,iadda),napar(:,iadda),size(nwert),size(nthpar),size(thparx(:,1)),size(nthtab))
       return
      END
...
!
       ...
       integer, save :: nser   = 1
       integer, save :: numor0 = 0
       integer       :: ia, ib, inew, ino
   
       character(len=8), save :: series_param = "par     "
       double precision, save :: par1, par2, dpar, par, noise
       double precision       :: sim_add_noise
        ...
        if(found('help    ')) then 
           write(6,*)'=============================================================================='
           write(6,*)'= creatser parname <par> par1 <p1> par2 <p2> n <n> [numor0 <numor0> [noise    '
           write(6,*)'=       creates a series of records using the first selcted item as template  '
           write(6,*)'=       the series contains a parameter <par> wjich varies from <p1> .. <p2>  '
           write(6,*)'=       the series has <n> items with numors numor0+1, +2, ...+n              '
           write(6,*)'=============================================================================='
           return
        endif
...
!          
      if(nsel.le.0) then
          call errsig(1999,"Need a selection for template (first item of list)$")
          return
      endif

      nser         = intval("n       ", nser         , inew)
      numor0       = intval("numor0  ", numor0       , inew)
      series_param = chrval("parname ", series_param , inew)
      par1         = getval("par1    ", par1         , inew)
      par2         = getval("par2    ", par2         , inew)
      noise        = getval("noise   ", 0d0          , inew)
      ...
      if( nbuf+nser > size(numor) ) then
          call errsig(1999,"Not enough record space for series$")
          return
      endif
      ...
      dpar = (par2-par1)/(max(1,nser-1))
      
      ia = isels(1)
      do ib=1,nser
        call txfera(ia,nbuf+ib)
        numor(nbuf+ib)  = numor0 + ib
        coment(nbuf+ib) = "ser: "//trim(series_param)//" template:"//coment(ia)(1:40)
        name(nbuf+ib)   = "ser:"//series_param(1:4) 
        if(noise == 0d0) then
           ywerte(:,nbuf+ib) = 0
           yerror(:,nbuf+ib) = 0
        else
           do ino = 1, nwert(ia)
              ywerte(ino,nbuf+ib) = sim_add_noise(dble(ywerte(ino,ia))/noise) * noise
              yerror(ino,nbuf+ib) = sqrt(ywerte(ino,ia)/noise) * noise 
              write(*,*)" test ",ia,ino, ywerte(ino,nbuf+ib) ,  ywerte(ino,ia)
           enddo
        endif

        par = par1 + (ib-1)*dpar
        call parset(series_param,sngl(par),nbuf+ib)        
      enddo

      ifits = 0
      nsel  = nser
      isels(1:nser) = [(ia,ia=nbuf+1,nbuf+nser)]

      write(*,'(a,a,2e14.7,a,i5,a,i5)') &
        trim(series_param)," =",par1,par2,"  series of records created on places: ",nbuf+1," ... ", nbuf+nser
      write(*,'(a)')"this series is now selected"
      write(*,*)
      nbuf = nbuf+nser

 
end subroutine create_ser

!===========================================================================================================
      subroutine serial_fit
!===========================================================================================================
...
!
       ...
       integer, parameter :: mexpa=100
     
       integer          :: isels_buf(size(isels))
       integer          :: nsel_buf
       integer          :: inew
       integer          :: nbuf0
       integer          :: nbuf1

       character(len=8) :: xparameter_name = "sel     "    


       character(len=8)  :: combinam
       character(len=8)  :: extraparams(mexpa)
       character(len=8)  :: extraid
       character(len=1)  :: cha
       integer           :: i,j,ii, ith, ni, is, ier
       integer           :: nextra
       real              :: xval, xpa
       ...
       if(found('help    ')) then 
           write(6,*)'=============================================================================='
           write(6,*)'= serial_fit [vs <parnam>]  [p1 <parnam> p2 ...                               '
           write(6,*)'=    fitting of selected data records one by one and storage of the paraemters'
           write(6,*)'=    in new records                                                           '
           write(6,*)'=    with p1 <parnam> p2 <>...                                                '
           write(6,*)'=    it is possible to inculde computed (parset-tted) params to the list recs '
           write(6,*)'=============================================================================='
           call fit
           return
        endif
 ...
 ! prepare new records
      xparameter_name = chrval("vs      ",xparameter_name ,inew)
      write(6,*)"VS: ", xparameter_name

 ! and check for extra info to be take form the parameter sections
      do i=1,mexpa
        write(extraid,'("p",i0)') i
        extraparams(i) =  chrval(extraid,extraid ,inew)
!       write(*,*)"extraparameter ", extraid,  extraparams(i) 
        if(inew == 0) exit
      enddo
      nextra = i-1
 ...
 ! initialize 
          ...
          if(ntheos.le.0) then
           call errsig(1999,"No theories activated !$")
           return
          endif
...
dt2:        do  j=1,nthpar(ith)
             if(ni >= size(nwert)) then
                call errsig(1999,"Not enough record buffers left to host fit params!$")
                return
             endif
             ni          = ni+1
             nwert(ni)    = 0
             ...
             coment(ni) = coment(isels_buf(1))(1:16)//"serfit:"//thenam(ith)//cha//thparn(j,ith)
             name(ni)   = combinam
             numor(ni)  = numor(isels(1))*100 + (ni-nbuf1)
             xname(ni)  = xparameter_name 
             yname(ni)  = combinam
             nopar(ni)  = 0
            enddo dt2
          enddo dt1
! and the ssq
             ...
             coment(ni) = coment(isels_buf(1))(1:16)//"serfit: ssq"
             name(ni)   = "ssq     "
             numor(ni)  = numor(isels(1))*100 + (ni-nbuf1)
             xname(ni)  = xparameter_name 
             yname(ni)  = "ssq     "
...
! and the extra-parameter
             do i=1, nextra
                ...
                combinam = extraparams(i) 
   
                coment(ni) = coment(isels_buf(1))(1:16)//"serfit:"//extraparams(i) 
                name(ni)   = combinam
                numor(ni)  = numor(isels(1))*100 + (ni-nbuf1)
                xname(ni)  = xparameter_name 
                yname(ni)  = extraparams(i) 
                nopar(ni)  = 0
             enddo
     


!! perform fits one by one !!
df:       do is = 1, nsel_buf
              nsel        = 1
              isels(nsel) = isels_buf(is)  
              ifits       = 0
              call fit
              call parget(xparameter_name,xval,isels(nsel),ier)
              if(ier .ne. 0) then
                xval = isels(nsel)
              endif 
! --and distribute--
...
dt12:         do  i=1,ntheos
                ith = nthtab(i)
dt22:           do  j=1,nthpar(ith)
                  ni           = ni+1
                  nwert(ni)    = min(nwert(ni)+1,size(xwerte(:,1)))
                  xwerte(nwert(ni),ni) = xval  
                  ywerte(nwert(ni),ni) = thparx(j,i)
                  yerror(nwert(ni),ni) = therro(j,i)
            enddo dt22
          enddo dt12
          ...
          nwert(ni)    = min(nwert(ni)+1,size(xwerte(:,1)))
          xwerte(nwert(ni),ni) = xval  
          ywerte(nwert(ni),ni) = ssq
          yerror(nwert(ni),ni) = 0

          do ii=1, nextra
            call parget(extraparams(ii) ,xpa, ifits(1),ier)
!            write(6,*)"ep: ",extraparams(ii), ifits(1),xpa

            ni           = ni+1
            nwert(ni)    = min(nwert(ni)+1,size(xwerte(:,1)))
            xwerte(nwert(ni),ni) = xval  
            ywerte(nwert(ni),ni) = xpa
            yerror(nwert(ni),ni) = 0
          enddo         
...
!
       ...
       character*8 ci
      dimension iparam(6),rparam(7),x(size(xinitial)),f(size(ferror)),xjac(size(ferror),size(xinitial)),  &
     &          xguess(size(xinitial)),xscale(size(xinitial)),fscale(size(ferror))
      dimension numv(size(inpar))
      dimension ermat(size(xinitial),size(ferror)), gmat(size(xinitial),size(xinitial)), ginv(size(xinitial),size(xinitial))

      dimension   xcenter(size(xinitial)), xstepsc(size(xinitial))
      character*8 xpname(size(xinitial))
          ...
          do 10 it =1,ntheos
            ...
            do 20 ip=1,npar
             if(thpsca(ip,it).ne.0) then
               nfit = nfit + 1
               if(nfit.gt.size(xinitial)) then
                 write(6,*)' too many fitparameters for current size(xinitial)=', &
     &                     size(xinitial)
                 return
               endif
...
!
      if(.not.found('noerrors ')) then
         ...
         enddo
! --- invertiere gmat ---
         call linrg(n,gmat,size(xinitial),ginv,size(xinitial))
! --- bilde g**-1 f  (f=xjac) ---
         do i=1,m
          do l=1,n
           sum = 0
...
!
        call couple(1)
        if(final_thc) call thc(size(xwerte(:,1)))
...
!
       ...
       real       ::   x(size(xinitial))
       real       ::   xguess(size(xinitial)),xscale(size(xinitial)),f(size(ferror))
       real       ::   xcenter(size(xinitial)), xstepsc(size(xinitial))
       dimension numv(size(inpar))
...
! compiler complains about nff unsused!!!!
       ...
       implicit none
       integer m, nff
       real      x(size(xinitial)),f(size(ferror))
...
! ---------> this if stmnt must correspond to the equival. in thc ! --
            ...
            if(lwrtfitdat) then
             ...
             if(m.ge.size(ferror)) then
               write(6,*)' func ==> no. of compared datapts ',m,        &
     &                   ' exhausts current dimension size(ferror)=',size(ferror)
               goto 111
             endif
           endif
!          -----> of if(xx.ge...
...
!      werden
!
       logical :: convolute = .false.
!       real :: extup = 0. ,extlow = 0.
       integer :: iadd, i, j, ier, m, n, np, npk, npoint, isigint, nconvo, isel, inew
...
!
       ...
       if(npoint.gt.size(xwerte(:,1))) then
          write(6,*)'thc warning: no. of points maximum is ',size(xwerte(:,1))
          npoint = size(xwerte(:,1))
       endif
!
...
! ------------------------------------------------
          if(npoint.gt.size(xwerte(:,1))) npoint = size(xwerte(:,1))
          n = npoint
...
!
         ...
         if(found('multiply')) then
           multflg(ntheos) = 1        ! multiply with preceeding theos result
           inaml           = 2
         elseif(found('insert')) then
           multflg(ntheos) = 2        ! insert preceeding theos result as x-value   
           inaml           = 3
         else
           multflg(ntheos) = 0
           inaml           = 1
...
!    *           ,vname(3)
          ...
          if(comand.eq.'theory  ') then
            ntheos = ntheos + 1
            if(ntheos.gt.size(nthtab)) then
               write(6,*)'too many theories ntheos=',ntheos,' max=',mtca
               ierrs = 703
               goto 999
            endif
!           ------- identify the theory ------
            do 7801 ith=1,size(nthpar)
...
! --------- look for coupleded items --------
            ...
            if(inames.ge.ilook) then
              do 7807 j=ilook,inames
                 ...
                 if(ncoup(i,ntheos).gt.size(thpafc(:,1,1))) then
...
!      -----------------------
       ...
       integer i, ith, kk, ier,l,j,lf
          ...
          do 150 i=1,ntheos
           ...
           elseif(multflg(i).eq.2) then
              write(buf1,*)'theory   ',thenam(ith),'   insert'
           else
...
!
       ...
       double precision :: xvector(size(xwerte(:,1))), yvector(size(xwerte(:,1))), yervector(size(xwerte(:,1)))
...
! --und nun hier das Einlesen 
     ...
     do ji=1,size(nwert)
      ...
      write(6,*)'inx reading record # ',ji, ' (',size(nwert),')'
...
!
      if(nk_act.gt.size(xwerte(:,1))) then
...
! und noch die eigentlichen Daten
      ...
      call parset('_xwidth ',xwerte(nint(kell)+1,nbuf)-xwerte(nint(kell),nbuf),nbuf)
...
!
       ...
       character*8  infile
       character*131 rline
       dimension ah(size(xwerte(:,1))),ak(size(xwerte(:,1))),al(size(xwerte(:,1))), &
                 anue(size(xwerte(:,1))),atemp(size(xwerte(:,1))),&
                 afeld(size(xwerte(:,1))),acounts(size(xwerte(:,1))),acorrcnt(size(xwerte(:,1))),           &
                 amonitor(size(xwerte(:,1))),extramon(size(xwerte(:,1))),atime(size(xwerte(:,1)))
!
! -- open the file -----------------------------------------------------
       if(inames.lt.2) then
         ierrs = 1
...
!! create destination
      ...
      xname(idest) = 'GHz     '
      yname(idest) = 'DOS(om) '
      if(Nom.lt.size(xwerte(:,1)) .and. Nom.gt.0) then
        nwert(idest) = Nom
      else
        nwert(idest) = size(xwerte(:,1))
      endif
! ..omega scale
      omega_scale = 1d9
      do i=1,nwert(idest)
...
!
       ...
       integer i,kp, km
       real xmirro, sum, t
       dimension y(size(xwerte(:,1))),y2(size(xwerte(:,1)))
!
        do 10 i=1,n
         y2(i) = 0
   10   continue
...
!
       ...
       integer i
        ...
        f0 = smirro(y,y2,xk0,nint(k1),nint(k2),n)
        do 100 i=1,10
          xk00 = xk0
          x1 = xk0 + 0.5
          x2 = xk0
          x3 = xk0 - 0.5
          f1 = smirro(y,y2,x1,nint(k1),nint(k2),n)
          f2 = smirro(y,y2,x2,nint(k1),nint(k2),n)
          f3 = smirro(y,y2,x3,nint(k1),nint(k2),n)
          f12 = (f1-f2)/(x1-x2)
          f23 = (f2-f3)/(x2-x3)
          a   = (f12-f23)/(x1-x3)
          xk0 = -0.5* (f12/a - x1 - x2)
...
! ----------> write data onto buffer <---------------------------------
         ...
         write(18,'(a,a,a,a,a,i14)')trim(finame(index(finame,'/',back=.true.)+1:)),&
...
501           format(2x,'   ',e14.7,5x,'   ',e14.7,5x,'   ',e14.7)
         ...
         close(18)
! ----------------------------------------------------------------------
         write(*,'(a)')'data and fits have been saved to: '//trim(outfile)
       return
999    continue
       call errsig(999,'file open failed...$')
       return
...
!
       ...
       call  fft2d(nfft,nfft,ca,size(ca(:,1)),ca,size(ca(:,1)))
...
! ---- reverse transformation back to scattering angle field ----
       call  fft2b(nfft,nfft,ca,size(ca(:,1)),ca,size(ca(:,1)))
!
! ---- return values ---------------------------------------------------
...
!      cosize(params(:,1))ison of original data and cleaned data on the same scale
...
!
       write(6,*)'starting fft....'
       call  rfft2d(nfft,nfft,ca,size(ca(:,1)),ca,size(ca(:,1)))
!!!!!  ai0= cabs(ca(1,1))/(1.-t)
       ai0=     (ca(1,1))/(1.-t)
!!!!!
...
!      integer ::  iwert, iwert2, itheo, iparnum
      integer ::  ibuf, iadr, iaddr, iiadr, inumr, l, myfun
      character*8 ::  pname
      integer :: i
      integer :: iwert
      integer :: iwert2
...
!        write(6,*) iwert2,min(iwert2,nwert(ibuf))
      ...
      if(compare(nam,'th_par ')) then
        ...
        iparnum = Nint(x(nx-1))
        itheo = Nint(x(nx  ))
        call index_check_th(iparnum,itheo)
        x(nx-1) = thparx(iparnum,itheo)
        nx = nx-1
        return
      endif
