...
module strings_module
  ...
  public :: split, tolower, toupper, match_wild
...
contains
  ...
  ! -------------------------------------------------------------------
  !>
  function tolower(str) result(lowercase)
    ...
    do i = 1, len(str)
       select case(str(i:i))
       case("A":"Z")
          lowercase(i:i) = achar(iachar(str(i:i))+32)
       case default
  ...
  ! -------------------------------------------------------------------
  !>
  function toupper(str) result(uppercase)
    character(len=*), intent(in) :: str
  ...
  ! -------------------------------------------------------------------
  !> split a string
  subroutine split(string, separator, words, nwords, max_len, max_words)
    integer, intent(in) :: max_len
    integer, intent(in) :: max_words
    character(len=*), intent(in) :: string
    character(len=*), intent(in) :: separator
    character(len=max_len), dimension(max_words), intent(out) :: words
  ...
  subroutine replace_char(string,oldchars,newchar)
    character(len=*), intent(inout) :: string
    character(len=*), intent(in)    :: oldchars
    character(len=1), intent(in)    :: newchar
    !
    integer   :: pos1, pos2, slen
    integer   :: i

    slen = len_trim(string)
    do i=1, len_trim(oldchars)
       pos1 = 1
       pos2 = 0
       do
          pos2 = index(string(pos1:),oldchars(i:i))
          if ( pos2<=0    ) exit
          pos1 = pos1 + pos2 - 1
          if ( pos1>slen ) exit
          string(pos1:pos1) = newchar
       end do
    end do
  ...
  !!
  !! @authors
  !!      Clive Page, userid: cgp  domain: le.ac.uk, 2003 (original code)
...
!         endif
         ...
         do i=2,iitems 
           ...
           if(cmd_item(i)(1:2)=="E:") then
              cycle
           endif
 
           if(cmd_item(i+1)(1:2)=="E:") then
             next_is_error = .true.
           endif
           ...
           if(ev_err==0) then 
              if(abs(ev_val) < 10*epsilon(ev_val)) then
                 write(99,'(a)',advance="no") "   "       !! ????  oder 0 (mit errorhandlng verknüft)
              else 
              ...
              cmd_item(i) =  string_replace(cmd_item(i),"_","\_")
              cmd_item(i) =  string_replace(cmd_item(i),"%","\%")
              write(99,'(a)', advance="no") cmd_item(i)(1:max(16,len_trim(cmd_item(i))))
           endif
           if(i<iitems) then
              if(next_is_error) then
                call evaluate(trim(cmd_item(i+2))//' ',ev_val,ev_err)
                if(abs(ev_val) >= 10*epsilon(ev_val)) write(99,'(a)', advance="no") " $\pm$"   !! ?? see obe 0
                next_is_error = .false.
              else
                write(99,'(a)', advance="no") " & "
              endif
