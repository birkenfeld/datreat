      FUNCTION th_roufric4 (x, pa, thnam, parnam, npar, ini, nopar ,params,napar,mbuf) 
!     ===================================================               
!                                                                       
! -------> rouse <--------                                              
!                                                                       
!                                    
      ...
      double precision :: fric0, fricn, nfric, fricc0, fricp0, frics
...
!        --------------> set the number of parameters                   
         ...
         parnam (5) = 'fric0'        ! friction modulation fmod= fric0+fricn*((i-fricp0)/narm)**nfric
         parnam (6) = 'fricn'        !      " 
         parnam (7) = 'frics'        !      " 
         parnam (8) = 'fricp0'       !      " 
         parnam (9) = 'nfric'        !      " 
         parnam (10)= 'fricc0'       ! friction modifer center
                                                                        
         parnam (11)  = 'aptrans '  ! amplitude step location in p                     
         parnam (12) = 'apwidth '  ! width of p step                    
         parnam (13) = 'ap1     '  ! start amplitude level                    
         parnam (14) = 'apnu    '  ! fermi(x**nu)        !                                                                       
         th_roufric4 = 0 
         RETURN 
      ENDIF 
!                                                                       
! ---- calculate theory here -----                                      
      ...
      if(x_is_tau) then                                                                
        sqt0   =       pericoroufric4_sqt(qz,0d0,f_arm,n_arm,Re_arm,Wl4, fric0, fricn, nfric)                                                       
        th_roufric4=  a0 * pericoroufric4_sqt(qz,tau,f_arm,n_arm,Re_arm,Wl4,fric0,fricn, nfric)/sqt0
      else
        th_roufric4=  a0 * pericoroufric4_sqt(qz,tau,f_arm,n_arm,Re_arm,Wl4, fric0, fricn, nfric)
      endif 

      if(diff .ne. 0d0) then
        th_roufric4 = th_roufric4 * exp(-qz*qz*diff*tau)
      endif 
...
double precision function pericoroufric4_sqt(q,t,f_arm,n_arm,Re_arm,Wl4, fric0, fricn, nfric)
!----------------------------------------------------------------------------------------------
...
!!
 ...
 double precision, save       :: fric00, fricn0, nfric0, fricc00, fricp00, frics0
...
eig:  if(  first_run                    &
     ...
     .or.  abs(frics-frics0)     > 1e-8 &
     .or.  abs(fricp0-fricp00)   > 1e-8 &
...
!      i1 = 1+n_arm*(j-1)+i
      i1 = 1+i
!      H(i1,i1) = H(i1,i1)/(fric0+fricn*((dble(i)+fricp0)/n_arm)**nfric)    !!! hier ??? !!!
      H(i1,i1) = H(i1,i1)/(fric0+fricn*atan(frics*((dble(i)+fricp0)/n_arm)**nfric))    !!! hier ??? !!!
   enddo
! enddo 
...
!$OMP END PARALLEL DO
        ...
        pericoroufric4_sqt = (1+f_arm*(s11+(f_arm-1)*s12))/N**2
...
!        pericoroufric4_sqt = pericoroufric4_sqt * exp( -q*q* effective_diffusion * t)
!     endif
!


end function pericoroufric4_sqt
